...
### parcels.crop_type stored mixed-case — adapters must normalize before comparing — 2026-04-19 (Request: chill-hours-display)

Discovered during chill-hours-display rollout: live `parcels.crop_type` values are stored mixed-case (`"Olivier"` instead of the expected `"olivier"`). The adapter's strict `cropType !== "olivier"` returned null on every real olive parcel, hiding chill_hours despite the engine computing 409h. Fix: `cropType.trim().toLowerCase() === "olivier"`. Considered normalizing on write at parcel update — rejected because (1) existing data already wrong and a migration is risky mid-SIAM prep, (2) other tables and adapters likely rely on the stored value as-is, and (3) localized read-side normalization is cheap. Lesson for future adapters: never compare `crop_type` (or `variety`) with strict equality — always lowercase + trim. Applies to the crop-reference loader path too (`getLocalCropReference` already does this internally).

### Phenological hour-counters unified on real-hourly backend computation — 2026-04-19 (Request: chill-hours-hourly-fetch)

All phenological temperature counters (chill hours, frost risk, heat stress, optimal flowering, growing hours, extreme heat, per-crop) are now computed server-side from real hourly Open-Meteo data, not client-side sine approximation from daily Tmin/Tmax. New `/api/weather/hour-counter` (generic) and `/api/weather/phenological-counters` (stage-aware) FastAPI endpoints. Stage definitions + thresholds live in per-crop referentiel `phenological_stages` block (OLIVIER/AGRUMES/AVOCATIER/PALMIER_DATTIER). Two-layer cache: `weather_hourly_data` (raw hourly, source of truth) + `weather_threshold_cache` (precomputed counts, hot path). Calibration step 2 `chill_hours` also sourced from the same hourly fetch — guarantees calibration page and weather tab show identical numbers. No sine fallback: Open-Meteo unavailability → `WeatherFetchError` → HTTP 502 → user retry. Frontend `PhenologicalTemperatureCounters` rewritten (806 → 158 lines), deleted `cropPhenologicalConfig` hardcode and client-side `calculateCountersForData`. Tradeoff: per-stage date-range customization removed (windows are now referentiel-defined, same for all parcels of a crop). Lazy migration: existing calibrations keep their sine-derived chill until next recalibration. Considered dumb-counter-only API with frontend computing stage math (B2) — rejected because single-source-of-truth guarantee requires server to own the computation.

### Module licensing is sysadmin-only, custom-invoice — 2026-04-24 (Request: admin-module-editor)

Module activation per organization is a licensing flag written only by system_admin from admin-app. Custom-invoice sales model: a call determines which modules each client gets, then sysadmin flips `organization_modules.is_active`. Considered and rejected: (a) org-admin self-toggle via `/settings/modules` — keeps an extra axis of state (visible-but-not-licensed) with no business reason and violates the memory rule that internal admin features live in admin-app; (b) subscription-plan auto-unlock via `required_plan` — not how the sales team sells. `required_plan`, `module_prices`, `organization_addons` stay in the schema for pricing display but do not gate access. `/settings/modules` in project/ becomes read-only with a contact-sales CTA.

### Module catalog collapsed to ~11 coarse SKUs — 2026-04-24 (Request: admin-module-editor)

Module SKUs are coarse (11 rows: core, chat_advisor, agromind_advisor, satellite, personnel, stock, production, fruit_trees, compliance, sales_purchasing, accounting, marketplace), not per-route. Each SKU's `navigation_items` JSONB lists the routes it unlocks. `core` and `chat_advisor` are bundled free (`is_required = true`); the other nine are sellable. Replaces (a) the 15-name snake_case seed in the schema migration (which was stale — missing chat, marketplace, compliance, production, etc.) and (b) the hardcoded 30+ kebab-case `MODULE_SECTIONS` array in `project/src/routes/_authenticated/(misc)/modules.tsx`. Considered and rejected: per-route fine catalog (30+ rows) — makes the sales conversation unwieldy ("do you want the /payments route?") and creates too many DB rows for sysadmin to manage. Existing `organization_modules` rows keyed on old ids will need a migration.

### project/ gating collapsed to a single source of truth — 2026-04-24 (Request: admin-module-editor)

Four gating paths exist today in project/: (1) `/modules` hub page reads hardcoded `MODULE_SECTIONS` + CASL only, (2) `ModuleGate` reads `navigation_items` via `useModuleBasedDashboard`, (3) Sidebar uses `isModuleActive(slug)` with hardcoded slugs, (4) `/settings/modules` toggles `organization_modules`. After refactor all four key off `organization_modules.is_active × modules.navigation_items` from the DB. Hardcoded `MODULE_SECTIONS`, `ROUTE_MODULE_MAP`, and `alwaysAllowed = ['/dashboard','/settings','/farm-hierarchy','/notifications']` in `ModuleGate.tsx` are removed — "always allowed" becomes `is_required = true` on the `core` module. `/modules` hub stays as a DB-driven launcher (keeps its UX value for Karim).
