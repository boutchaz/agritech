...
import { useAIPlan, useAIPlanInterventions, useExecuteAIPlanIntervention, useRegenerateAIPlan } from '@/hooks/useAIPlan'
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-white dark:bg-gray-800 p-4 rounded-xl border border-gray-200 dark:border-gray-700">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h3 className="font-semibold text-gray-900 dark:text-white">Plan Status</h3>
            <p className="text-sm text-gray-500 dark:text-gray-400 capitalize">{plan?.status || 'No active plan'}</p>
          </div>
        </div>
        <button
          type="button"
      ...
      {!interventions || interventions.length === 0 ? (
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-12 text-center">
          ...
          <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-2">No Interventions Scheduled</h3>
          <p className="text-gray-500 dark:text-gray-400">Generate a new plan to see scheduled interventions.</p>
        </div>
      ) : (
