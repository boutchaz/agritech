    ...
    return (
      <div className="rounded-xl border border-amber-200 bg-amber-50 p-6 shadow-sm dark:border-amber-800/30 dark:bg-amber-900/20">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h3 className="font-semibold text-amber-900 dark:text-amber-100">{t('ai.weather.riskUnavailable', 'Risk analysis unavailable')}</h3>
            <p className="mt-1 text-sm text-amber-700 dark:text-amber-300">
              {t('ai.weather.riskUnavailableDescription', 'Weather analytics remain available, but the crop disease risk model could not be loaded for this parcel.')}
            </p>
          </div>
        </div>
      </div>
    ...
    <div className="space-y-4 rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h3 className="flex items-center gap-2 text-lg font-semibold text-gray-900 dark:text-white">
            ...
            {t('ai.weather.riskBriefTitle', 'AI Weather Risk Brief')}
          </h3>
          <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
            {t('ai.weather.riskBriefDescription', 'Crop-specific disease exposure inferred from the latest parcel weather conditions.')}
          </p>
        </div>
        <span className="rounded-full bg-blue-50 px-3 py-1 text-xs font-medium text-blue-700 dark:bg-blue-900/30 dark:text-blue-200">
          {t('ai.weather.crop', 'Crop: {{crop}}', { crop: data.crop_type || 'Unknown' })}
        </span>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        <div className="rounded-lg bg-gray-50 p-4 dark:bg-gray-900/50">
          <div className="flex items-center gap-2 text-xs font-medium uppercase tracking-wide text-gray-500 dark:text-gray-400">
            ...
            {t('ai.weather.activeRisks', 'Active Risks')}
          </div>
          <div className="mt-2 text-3xl font-bold text-gray-900 dark:text-white">{activeRisks.length}</div>
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            {activeRisks.length > 0 ? t('ai.weather.activeRisksDescription', 'Conditions currently match one or more disease profiles.') : t('ai.weather.noActiveRisks', 'No active disease signals from the latest weather sample.')}
          </p>
        </div>
        ...
        <div className="rounded-lg bg-gray-50 p-4 dark:bg-gray-900/50">
          <div className="flex items-center gap-2 text-xs font-medium uppercase tracking-wide text-gray-500 dark:text-gray-400">
            ...
            {t('ai.weather.latestTemperature', 'Latest Temperature')}
          </div>
          <div className="mt-2 text-3xl font-bold text-gray-900 dark:text-white">
            {currentTemperature != null ? `${currentTemperature.toFixed(1)}°C` : 'N/A'}
          </div>
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            {data.weather.date ? t('ai.weather.measuredOn', 'Measured on {{date}}', { date: data.weather.date }) : t('ai.weather.noRecentReading', 'No recent parcel weather reading.')}
          </p>
        </div>
        ...
        <div className="rounded-lg bg-gray-50 p-4 dark:bg-gray-900/50">
          <div className="flex items-center gap-2 text-xs font-medium uppercase tracking-wide text-gray-500 dark:text-gray-400">
            ...
            {t('ai.weather.latestHumidity', 'Latest Humidity')}
          </div>
          <div className="mt-2 text-3xl font-bold text-gray-900 dark:text-white">
            {currentHumidity != null ? `${currentHumidity.toFixed(0)}%` : 'N/A'}
          </div>
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            {t('ai.weather.humidityDescription', 'Used to evaluate crop-specific humidity thresholds.')}
          </p>
        </div>
      </div>
      ...
      {data.risks.length === 0 ? (
        <div className="rounded-lg border border-dashed border-gray-300 p-4 text-sm text-gray-600 dark:border-gray-600 dark:text-gray-300">
          {t('ai.weather.noDiseaseModels', 'No disease models are configured yet for this crop, so only weather analytics are shown below.')}
        </div>
        ...
        <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-800 dark:border-emerald-800/30 dark:bg-emerald-900/20 dark:text-emerald-200">
          {t('ai.weather.noActiveProfiles', 'Current temperature and humidity do not activate any configured disease risk profile for this crop.')}
        </div>
      ) : (
          ...
          {activeRisks.map((risk) => (
            ...
            >
              ...
              <div className="mt-3 grid grid-cols-1 gap-2 text-sm text-gray-700 dark:text-gray-300">
                <p>
                  {t('ai.weather.temperatureWindow', 'Temperature window: {{min}}°C to {{max}}°C', { min: risk.temperature_range.min ?? 'N/A', max: risk.temperature_range.max ?? 'N/A' })}
                </p>
                <p>
                  {t('ai.weather.humidityThreshold', 'Humidity threshold: {{value}}%', { value: risk.humidity_threshold != null ? `${risk.humidity_threshold}` : 'N/A' })}
                </p>
                {risk.treatment_product && (
                  <p>{t('ai.weather.suggestedTreatment', 'Suggested treatment: {{treatment}}', { treatment: `${risk.treatment_product}${risk.treatment_dose ? ` • ${risk.treatment_dose}` : ''}` })}</p>
                )}
                {risk.treatment_timing && <p>{t('ai.weather.timing', 'Timing: {{timing}}', { timing: risk.treatment_timing })}</p>}
              </div>
            </div>
          ))}
        </div>
...
const AIWeatherPage = () => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-bold text-gray-900 dark:text-white">{t('ai.weather.pageTitle', 'AI Weather Analysis')}</h2>
        <p className="text-sm text-gray-600 dark:text-gray-400">
          {t('ai.weather.pageSubtitle', 'Weather analytics, forecast trends, and crop risk signals for this parcel.')}
        </p>
      </div>
      ...
      {hasBoundary ? (
        ...
        <div className="rounded-xl bg-white py-12 text-center dark:bg-gray-800">
          ...
          <p className="mb-2 text-gray-600 dark:text-gray-400">
            {t('ai.weather.noBoundary', 'Parcel location data is required for weather analysis.')}
          </p>
          <p className="text-sm text-gray-500 dark:text-gray-500">
            {t('ai.weather.noBoundaryHint', 'Add or correct the parcel boundary to enable forecast, climate, and risk analysis.')}
          </p>
        </div>
      )}
    </div>
