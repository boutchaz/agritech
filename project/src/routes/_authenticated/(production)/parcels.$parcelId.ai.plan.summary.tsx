...
const AIPlanSummaryPage = () => {
  ...
  const { data: summary, isLoading: isSummaryLoading } = useAIPlanSummary(parcelId, !!plan);
  const validatePlan = useValidateAIPlan(parcelId);

  if (isLoading || (isSummaryLoading && !!plan)) {
    return (
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-8">
        ...
        {!plan ? (
          <p className="text-gray-500 dark:text-gray-400 text-center py-8">Aucune synthese de plan disponible.</p>
        ) : (
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div className="bg-gray-50 dark:bg-gray-900/50 p-4 rounded-lg">
                <h4 className="font-medium text-gray-900 dark:text-white mb-2">Interventions totales</h4>
                <p className="text-2xl font-bold text-gray-900 dark:text-white">{summary?.total_interventions ?? 0}</p>
              </div>
              <div className="bg-gray-50 dark:bg-gray-900/50 p-4 rounded-lg">
                <h4 className="font-medium text-gray-900 dark:text-white mb-2">Executees</h4>
                <p className="text-2xl font-bold text-green-600 dark:text-green-400">{summary?.executed ?? 0}</p>
              </div>
              <div className="bg-gray-50 dark:bg-gray-900/50 p-4 rounded-lg">
                <h4 className="font-medium text-gray-900 dark:text-white mb-2">Planifiees</h4>
                <p className="text-2xl font-bold text-blue-600 dark:text-blue-400">{summary?.planned ?? 0}</p>
              </div>
              <div className="bg-gray-50 dark:bg-gray-900/50 p-4 rounded-lg">
