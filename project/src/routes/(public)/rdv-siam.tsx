    ...
    <div className="min-h-screen bg-[#f9f7f2] text-[#1a1a1a]">
      <section className="bg-[#1d6b3a] px-6 py-12 text-center">
        <span className="inline-block rounded-full bg-[#c8892a] px-4 py-1 text-xs font-medium uppercase tracking-[0.12em] text-white">
          {t('siamRdv.hero.badge', { defaultValue: 'SIAM 2026 · Meknes' })}
        </span>
        <h1 className="mt-5 text-3xl font-semibold text-white md:text-5xl">
          {t('siamRdv.hero.title', { defaultValue: 'Reservez votre rendez-vous avec AGROGINA' })}
        </h1>
        <p className="mx-auto mt-4 max-w-xl text-sm text-white/80 md:text-base">
          {t('siamRdv.hero.subtitle', {
            defaultValue:
              'Decouvrez comment notre plateforme AgriTech transforme la gestion de vos exploitations. Demonstration personnalisee sur notre stand.',
          })}
        </p>
      </section>
      ...
      <main className="mx-auto max-w-3xl px-4 py-8 md:px-6">
        {!submitted ? (
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            ...
            <button type="submit" className="w-full rounded-lg bg-[#1d6b3a] px-4 py-3 font-medium text-white hover:bg-[#2e8b50]">
              {isSubmittingToApi
