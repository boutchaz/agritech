...
const TaskForm = ({
  ...
  useEffect(() => {
    if (!task && selectedParcel && formData.parcel_id) {
      if (selectedParcel.crop_type && formData.task_type) {
        ...
        const isAutoGenerated = !currentTitle || currentTitle.includes(cropType) || currentTitle.includes(selectedParcel.name);
  ...
  }, [formData.parcel_id, formData.task_type, selectedParcel, task, setValue]);
