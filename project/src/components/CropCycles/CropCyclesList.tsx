...
const cropCycleSchema = z.object({
  ...
  cycle_type: z.enum(['annual', 'perennial', 'multi_harvest', 'continuous']).optional(),
...
export function CropCyclesList() {
  ...
  const { data: existingCrops = [] } = useCrops({ farmId: selectedFarmId || undefined, enabled: !!selectedFarmId });
  ...
  useEffect(() => {
    ...
    if (templateId && plantingDate && !harvestStart && !harvestEnd) {
      ...
      if (template && template.typical_duration_months) {
        ...
        harvestStartDate.setMonth(harvestStartDate.getMonth() + template.typical_duration_months);
  ...
  useEffect(() => {
    ...
    if (parcelId && plantingDate && harvestEnd) {
      ...
      const overlappingCycle = cropCycles.find(cycle => {
        ...
        if (!['land_prep', 'growing', 'harvesting', 'planned'].includes(cycle.status)) return false;

        const cycleStart = cycle.planting_date ? new Date(cycle.planting_date) : null;
        const cycleEnd = cycle.expected_harvest_end ? new Date(cycle.expected_harvest_end) : null;
  ...
  }, [form.watch('parcel_id'), form.watch('planting_date'), form.watch('expected_harvest_end'), cropCycles, editingCycle]);
  ...
  const generateCode = () => {
    ...
    const template = templates.find(t => t.id === form.getValues('template_id')) || 
                    Object.values(MOROCCO_CROP_TEMPLATES).find(
      ...
      if (editingCycle) {
        ...
        if (!cropId && data.crop_type && data.farm_id) {
          const matchingCrop = existingCrops.find(
            (crop: any) => crop.name?.toLowerCase().includes(data.crop_type.toLowerCase()) ||
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-end">
        <div className="flex flex-wrap gap-4 flex-1">
          ...
          <Select value={filterCycleType || '__all__'} onValueChange={(v) => setFilterCycleType(v === '__all__' ? '' : v)}>
            <SelectTrigger className="w-[160px]">
              <SelectValue placeholder={t('cropCycles.filter.type', 'All Types')} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="__all__">{t('common.all', 'All Types')}</SelectItem>
              <SelectItem value="annual">{t('cropCycles.type.annual', 'Annual')}</SelectItem>
              <SelectItem value="perennial">{t('cropCycles.type.perennial', 'Perennial')}</SelectItem>
              <SelectItem value="multi_harvest">{t('cropCycles.type.multiHarvest', 'Multi-Harvest')}</SelectItem>
            </SelectContent>
          </Select>

          <Select value={filterSeason || '__all__'} onValueChange={(v) => setFilterSeason(v === '__all__' ? '' : v)}>
            <SelectTrigger className="w-[150px]">
              <SelectValue placeholder={t('cropCycles.filter.season', 'All Seasons')} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="__all__">{t('common.all', 'All Seasons')}</SelectItem>
      ...
      {isLoading ? (
        ...
        <EmptyState
          ...
          description={t('cropCycles.empty', 'No crop cycles found. Create your first cycle to start tracking production.')}
          action={
            canManage
              ? {
                  ...
                  <div className="flex justify-between items-start">
                    <div className="space-y-1.5">
                      <div className="flex items-center gap-2">
                        <CardTitle 
                          ...
                          onClick={() => navigate({ to: '/crop-cycles/$cycleId', params: { cycleId: cycle.id } })}
                        >
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        <DropdownMenuItem onClick={() => navigate({ to: '/crop-cycles/$cycleId', params: { cycleId: cycle.id } })}>
                          <Eye className="h-4 w-4 mr-2" /> {t('common.view', 'View Details')}
                        </DropdownMenuItem>
        ...
        <Card>
          <CardHeader>
            <CardTitle>{t('cropCycles.timeline', 'Production Timeline')}</CardTitle>
            <CardDescription>{t('cropCycles.timelineDesc', 'Visual overview of all active crop cycles')}</CardDescription>
          </CardHeader>
            ...
            <div className="min-w-[800px]">
              <div className="grid grid-cols-12 gap-1 mb-4 border-b pb-2 text-sm font-medium text-muted-foreground text-center">
                {Array.from({ length: 12 }).map((_, i) => (
                  <div key={i}>{new Date(0, i).toLocaleString('default', { month: 'short' })}</div>
                ))}
              </div>
              
              <div className="space-y-2">
                {filteredCycles.map(cycle => {
                  const start = cycle.planting_date ? new Date(cycle.planting_date) : new Date();
                  const end = cycle.expected_harvest_end ? new Date(cycle.expected_harvest_end) : new Date(start.getTime() + 90 * 24 * 60 * 60 * 1000);
                  
                  const startMonth = start.getMonth();
                  const durationMonths = Math.max(1, (end.getTime() - start.getTime()) / (1000 * 60 * 60 * 24 * 30));
                  const startPercent = (startMonth / 12) * 100;
                  const widthPercent = Math.min((durationMonths / 12) * 100, 100 - startPercent);

                  return (
                    <div key={cycle.id} className="relative h-10 flex items-center bg-muted/20 rounded hover:bg-muted/40 transition-colors">
                      <div className="w-48 shrink-0 px-2 text-sm font-medium truncate border-r mr-2">
                        {cycle.cycle_name || cycle.crop_type}
                        <div className="text-xs text-muted-foreground font-normal">{cycle.cycle_code}</div>
                      </div>
                      <div className="flex-1 relative h-full flex items-center">
                        <div
                          className={`absolute h-6 rounded px-2 text-xs text-white flex items-center whitespace-nowrap overflow-hidden
                            ${cycle.status === 'completed' ? 'bg-purple-500' : 
                              cycle.status === 'harvesting' ? 'bg-blue-500' : 
                              cycle.status === 'growing' ? 'bg-green-500' : 'bg-gray-400'}`}
                          style={{
                            left: `${startPercent}%`,
                            width: `${widthPercent}%`,
                          }}
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            {!editingCycle && templates.length > 0 && (
              <div className="p-3 bg-muted/30 rounded-md border border-dashed">
                <Label className="mb-1.5 block text-primary font-medium">{t('cropCycles.form.template', 'Crop Template (Optional)')}</Label>
                <Select
                  value={form.watch('template_id') || '__none__'}
                  onValueChange={handleTemplateChange}
                >
                  <SelectTrigger>
                    <SelectValue placeholder={t('cropCycles.form.selectTemplate', 'Select a template to auto-fill...')} />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="__none__">{t('common.none', 'No Template')}</SelectItem>
