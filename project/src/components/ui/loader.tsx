      ...
      <div className="p-3 sm:p-4 lg:p-6 space-y-6">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
          ...
          {Array.from({ length: 6 }).map((_, skIdx) => (
            <TableRowSkeleton key={"sk-" + skIdx} columns={4} />
          ))}
        </div>
      </div>
    </div>
    ...
    <div className={cn('space-y-3 py-6 px-4 animate-in fade-in duration-300', className)}>
      ...
      <div className="space-y-3">
        {Array.from({ length: 4 }).map((_, rowIdx) => (
          <div key={"sk-" + rowIdx} className="flex items-center gap-3">
            <Skeleton className="h-10 w-10 rounded-lg" />
            <div className="flex-1">
              <Skeleton className={cn('h-4 mb-1', rowIdx % 2 === 0 ? 'w-3/4' : 'w-1/2')} />
