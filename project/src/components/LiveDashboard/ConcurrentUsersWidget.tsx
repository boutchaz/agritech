...
const ConcurrentUsersWidget = ({
  ...
  const getStatusColor = (lastActivity: string) => {
    const minutesAgo = (now - new Date(lastActivity).getTime()) / 60000;
        ...
        <div className="relative flex items-center justify-between">
          ...
          <div className="flex -space-x-3">
            {users.slice(0, 4).map((user, _index) => (
              <div
                key={user.id}
                className="w-10 h-10 rounded-xl bg-gradient-to-br from-cyan-500 to-blue-500 flex items-center justify-center text-white text-[10px] font-medium border-4 border-slate-50 dark:border-slate-900 shadow-sm"
                title={user.name}
              >
            ...
            {users.length > 4 && (
              <div className="w-10 h-10 rounded-xl bg-slate-200 dark:bg-slate-700 flex items-center justify-center text-slate-600 dark:text-slate-300 text-[10px] font-medium border-4 border-slate-50 dark:border-slate-900 shadow-sm">
                +{users.length - 4}
              </div>
            )}
          </div>
            ...
            {users.map((user) => (
              ...
              >
                ...
                <div className="text-right flex-shrink-0 ml-3">
                  <div className="text-[8px] font-medium text-cyan-600 dark:text-cyan-400 bg-cyan-50 dark:bg-cyan-900/30 px-2 py-1 rounded-lg uppercase tracking-widest border border-cyan-100 dark:border-cyan-800">
