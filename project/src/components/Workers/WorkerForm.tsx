...
const createWorkerSchema = (t: any) =>
  z
    .object({
      ...
      email: z
        .string()
        .email(t("workers.form.validation.emailInvalid"))
        .optional()
        .or(z.literal("")),
...
const WorkerForm = ({
  ...
  useEffect(() => {
    if (worker) {
      reset({
        ...
        per_unit_rate: worker.per_unit_rate ?? undefined,
        metayage_type: worker.metayage_type ?? undefined,
        metayage_percentage: worker.metayage_percentage ?? undefined,
        calculation_basis: worker.calculation_basis ?? undefined,
  ...
  useEffect(() => {
    if (workerType === "metayage" && metayageType) {
      const option = METAYAGE_TYPE_OPTIONS.find(
        (o) => o.value === metayageType,
      );
      ...
      if (
        ...
        try {
          ...
          if (result.success) {
            ...
            toast.error(
              t("workers.form.errors.workerCreatedAccessFailedTryUsers"),
            );
          }
        } catch (error: unknown) {
  ...
  const headerContent = (
    <div className="flex items-center gap-3 text-gray-900 dark:text-white">
      ...
      <span>
        {isEditing ? t("workers.form.title.edit") : t("workers.form.title.add")}
      </span>
    </div>
  );
      ...
      <Button type="submit" form="worker-form" disabled={isSubmitting}>
        {isSubmitting ? (
          ...
          <>
            ...
            <span>
              {isEditing
                ? t("workers.form.buttons.update")
                : t("workers.form.buttons.create")}
            </span>
          </>
        )}
      </Button>
    ...
    >
            ...
            <div>
              ...
              <ul className="text-sm text-red-700 dark:text-red-300 space-y-1 list-disc list-inside">
                {Object.entries(errors).map(([field, error]) => (
                  <li key={field}>
                    {error?.message ||
                      `${t("workers.form.errors.errorInField")}: ${field}`}
                  </li>
                ))}
              </ul>
            </div>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white"
                  placeholder={t(
                    "workers.form.placeholders.metayagePercentage",
                  )}
                />
                {errors.metayage_percentage && (
                  <p className="text-red-600 text-sm mt-1">
                    {errors.metayage_percentage.message}
                  </p>
                )}
              </div>
            </div>
            ...
            <div className="bg-blue-50 dark:bg-blue-900/20 p-3 rounded-lg text-sm text-blue-800 dark:text-blue-200">
              <p className="font-medium mb-1">
                ℹ️ {t("workers.form.metayageInfo.title")}
              </p>
              <ul className="list-disc list-inside space-y-1">
                <li>
                  <strong>
                    {t("workers.form.metayageInfo.khammass.title")}:
                  </strong>{" "}
                  {t("workers.form.metayageInfo.khammass.description")}
                </li>
                <li>
                  <strong>{t("workers.form.metayageInfo.rebaa.title")}:</strong>{" "}
                  {t("workers.form.metayageInfo.rebaa.description")}
                </li>
                <li>
                  <strong>
                    {t("workers.form.metayageInfo.tholth.title")}:
                  </strong>{" "}
                  {t("workers.form.metayageInfo.tholth.description")}
                </li>
              </ul>
            </div>
          </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            >
              <option value="">{t("workers.form.options.select")}</option>
              <option value="cash">
                {t("workers.form.paymentMethods.cash")}
              </option>
              <option value="bank_transfer">
                {t("workers.form.paymentMethods.bankTransfer")}
              </option>
              <option value="check">
                {t("workers.form.paymentMethods.check")}
              </option>
              <option value="mobile_money">
                {t("workers.form.paymentMethods.mobileMoney")}
              </option>
            </select>
          </div>
        ...
        <div className="mt-4 bg-blue-50 dark:bg-blue-900/20 p-3 rounded-lg text-sm text-blue-800 dark:text-blue-200">
          <p className="font-medium mb-1">
            ℹ️ {t("workers.form.paymentFrequencyInfo.title")}:
          </p>
          <ul className="list-disc list-inside space-y-1">
            {workerType === "fixed_salary" && (
              <>
                <li>
                  <strong>
                    {t("workers.form.paymentFrequencyInfo.monthly")}:
                  </strong>{" "}
                  {t("workers.form.paymentFrequencyInfo.monthlyDescription")}
                </li>
                <li>
                  <strong>
                    {t("workers.form.paymentFrequencyInfo.daily")}:
                  </strong>{" "}
                  {t("workers.form.paymentFrequencyInfo.dailyDescription")}
                </li>
              </>
            )}
            {workerType === "daily_worker" && (
              <>
                <li>
                  ...
                  {t(
                    "workers.form.paymentFrequencyInfo.dailyWorkerDailyDescription",
                  )}
                </li>
                <li>
                  <strong>
                    {t("workers.form.paymentFrequencyInfo.perTask")}:
                  </strong>{" "}
                  {t("workers.form.paymentFrequencyInfo.perTaskDescription")}
                </li>
              </>
            )}
            {workerType === "metayage" && (
              <li>
                <strong>
                  {t("workers.form.paymentFrequencyInfo.harvestShare")}:
                </strong>{" "}
                {t("workers.form.paymentFrequencyInfo.harvestShareDescription")}
              </li>
            )}
          </ul>
        </div>
