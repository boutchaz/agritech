    ...
    >
      ...
      <View style={[styles.card, { backgroundColor: themeColors.surfaceLowest }]}>
        <View style={[styles.iconWrap, { backgroundColor: themeColors.brandContainer + '30' }]}>
          <Ionicons name="sparkles" size={28} color={themeColors.brandPrimary} />
        </View>
        <Text style={[styles.title, { color: themeColors.textPrimary }]}>Agromind IA</Text>
        <Text style={[styles.subtitle, { color: themeColors.textSecondary }]}>
          AI-powered crop diagnostics, recommendations, and intelligent alerts for this parcel
        </Text>
      </View>

      <View style={styles.featureList}>
        {[
          { icon: 'scan-outline', title: 'Crop Diagnostics', desc: 'AI analysis of crop health and disease detection' },
          { icon: 'bulb-outline', title: 'Recommendations', desc: 'Smart suggestions for fertilization, irrigation, and treatments' },
          { icon: 'alert-circle-outline', title: 'Smart Alerts', desc: 'Predictive alerts for pest risks and weather impacts' },
        ].map((item) => (
