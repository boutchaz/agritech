-- AgriTech Database Setup Script
-- This script creates all necessary tables and policies for the agricultural management system
...
-- Create policies for read-only tables (accessible to all authenticated users)
...
CREATE POLICY "Users can manage employees in their farms"
  ON employees FOR ALL
  TO authenticated
  USING (farm_id IN (SELECT id FROM farms WHERE user_id = auth.uid()))
  WITH CHECK (farm_id IN (SELECT id FROM farms WHERE user_id = auth.uid()));

CREATE POLICY "Users can manage day laborers in their farms"
...
-- Insert some default crop types for testing
INSERT INTO crop_types (name, description)
VALUES
  ('Arboriculture', 'Arbres fruitiers'),
  ('Maraichage', 'Légumes et cultures maraîchères'),
  ('Élevage', 'Animaux d''élevage'),
  ('Aquaculture', 'Poissons et crustacés')
ON CONFLICT DO NOTHING;