...
class CloudMaskingService:
    ...
    def mask_clouds_s2(image: ee.Image) -> ee.Image:
        """
        Apply cloud masking to Sentinel-2 SR image using QA60 + SCL bands.
        Safe for use inside collection.map() — no getInfo() calls.
        """
        ...
        # S2_SR_HARMONIZED always includes SCL — select it directly.
        # This is a server-side operation safe for .map()
    ...
    def calculate_cloud_coverage_in_aoi(
        image: ee.Image, aoi: ee.Geometry, buffer_meters: Optional[float] = None
    ) -> float:
        """
        ...
        try:
            ...
            cloud_percentage = (
                ee.Number(cloud_count).divide(ee.Number(total_count)).multiply(100)
            )
            ...
            logger.info(
                f"Cloud coverage in AOI: {result:.2f}% (buffer: {buffer_meters}m)"
            )
            return result
    ...
    def create_cloud_probability_mask(
        image: ee.Image, threshold: float = 50
    ) -> ee.Image:
        """
