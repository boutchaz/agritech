...
describe('CalibrationBlocksReviewAdapter', () => {
  let adapter: CalibrationBlocksReviewAdapter;

  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [CalibrationBlocksReviewAdapter],
    }).compile();
    adapter = module.get(CalibrationBlocksReviewAdapter);
  });

  it('transform() returns schema_version v2 with all blocks', () => {
    const result = adapter.transform(buildSnapshotInput());

    expect(result.schema_version).toBe('calibration-review/v2');
