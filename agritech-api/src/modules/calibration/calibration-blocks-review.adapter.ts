...
import type {
  ...
  AlternanceLabel,
  SeasonBadge,
} from "./dto/calibration-blocks-review.dto";
...
@Injectable()
export class CalibrationBlocksReviewAdapter {
  private readonly logger = new Logger(CalibrationBlocksReviewAdapter.name);

  transform(input: CalibrationSnapshotInput): CalibrationBlocksReviewView {
    this.logger.debug(`Transforming calibration snapshot ${input.calibration_id} to blocks v2`);
  ...
  private buildBlockB(input: CalibrationSnapshotInput): BlockBAnalyse {
    ...
    return {
      ...
      spatial_patterns: null, // Phase 2 — confidence threshold TBD
  ...
  private buildHeatmap(step7: JsonRecord, step1: JsonRecord): BlockBAnalyse["heatmap"] {
    ...
    const hasValidSummerImage = eviSeries.some((point) => {
      ...
      if (!d) return false;
      const month = new Date(d).getMonth(); // 0-indexed: 6=July, 7=August
    ...
    // Find closest EVI date in summer window for date_image
