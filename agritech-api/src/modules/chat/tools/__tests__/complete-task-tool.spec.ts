...
describe('complete_task tool', () => {
  ...
  beforeEach(async () => {
    ...
    const mockSupabase = {
      from: jest.fn((table: string) => {
        if (table === 'tasks') return buildLookupChain({ id: taskId, title: 'Irriguer B3', status: 'in_progress' });
  ...
  it('should show task title and status transition in preview', async () => {
    const result = await service.executeTool(
      'complete_task',
      { mode: 'preview', task_id: taskId, notes: 'Bien fait' },
      userId, orgId,
    );

    expect(result.success).toBe(true);
    expect(result.preview_data.task_title).toBe('Irriguer B3');
    expect(result.preview_data.current_status).toBe('in_progress');
    expect(result.preview_data.new_status).toBe('completed');
    expect(mockPendingActionService.upsert).toHaveBeenCalled();
  });

  it('should return error for already-completed task', async () => {
    const mockSupabase = {
      from: jest.fn(() => buildLookupChain({ id: taskId, title: 'Irriguer B3', status: 'completed' })),
    };
