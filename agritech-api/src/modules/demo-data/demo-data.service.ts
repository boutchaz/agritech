...
export class DemoDataService {
  ...
  async seedDemoData(organizationId: string, userId: string): Promise<void> {
    ...
    try {
      ...
      const utilities = await this.createDemoUtilities(organizationId, farm.id, parcels);
  ...
  private async createDemoUtilities(
    ...
    // Create diverse utilities across multiple months
    const utilities = [
      ...
      // Water bills (monthly recurring)
      {
        organization_id: organizationId,
        farm_id: farmId,
        parcel_id: parcels[0]?.id, // Linked to olive parcel
        type: 'water',
      ...
      },
      {
        organization_id: organizationId,
        farm_id: farmId,
        parcel_id: parcels[1]?.id, // Linked to citrus parcel
        type: 'water',
        provider: 'ONEP (Office National de l\'Eau Potable)',
  ...
  async getDataStats(organizationId: string): Promise<Record<string, number>> {
    ...
    const tables = [
      ...
      'payments', 'journal_entries', 'utilities',
