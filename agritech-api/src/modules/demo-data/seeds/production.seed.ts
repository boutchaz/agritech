...
export class ProductionSeedService {
  ...
  async createDemoHarvests(
    ...
    if (harvestsError) {
      this.logger.error(`Failed to create demo harvests: ${harvestsError.message}`);
      return [];
    }
  ...
  async createDemoReceptionBatches(
    ...
    if (harvests.length === 0) {
      this.logger.warn('Cannot create reception batches: no harvests available');
      return [];
    }
    ...
    if (batchesError) {
      this.logger.error(`Failed to create demo reception batches: ${batchesError.message}`);
      return [];
    }
  ...
  async createDemoProductionCosts(
    ...
    if (costsError) {
      this.logger.error(`Failed to create demo production costs: ${costsError.message}`);
      return [];
    }
  ...
  async createDemoProductionRevenues(
    ...
    if (harvests.length === 0 || receptionBatches.length === 0) {
      this.logger.warn('Cannot create production revenues: no harvests or batches available');
      return [];
    }
    ...
    if (revenuesError) {
      this.logger.error(`Failed to create demo production revenues: ${revenuesError.message}`);
      return [];
    }
